...
<body class="bg-[#0f0608] text-white antialiased">
    ...
    <nav class="bg-[#130609] border-b border-white/5 sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-6 h-16 flex items-center gap-5">
            ...
            <form action="{{ route('listings.search') }}" method="GET" class="flex-1 max-w-lg">
                <div
                    class="flex bg-white/5 border border-white/10 rounded-lg overflow-hidden h-10 focus-within:border-[#7D1A2E] transition-colors">
                    <input type="text" name="q" value="{{ request('q') }}"
                        placeholder="Cari produk, toko, atau kategori..."
                        class="flex-1 bg-transparent px-4 text-sm text-white placeholder-white/30 outline-none">
                    <button type="submit"
                        class="bg-[#7D1A2E] hover:bg-[#9B2035] px-5 text-sm font-semibold text-white transition-colors shrink-0">
            ...
            <div class="ml-auto flex items-center gap-2">
                ...
                @auth
                    ...
                    <div class="relative">
                        ...
                        @if ($unread > 0)
                            <span
                                class="absolute -top-1 -right-1 bg-[#C0392B] text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
                                {{ $unread > 9 ? '9+' : $unread }}
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open"
                            ...
                            @else
                                <span
                                    class="w-5 h-5 rounded-full bg-[#7D1A2E] flex items-center justify-center text-[10px] font-bold text-white">
                        ...
                        <div x-show="open" @click.outside="open = false" x-transition
                            ...
                            <div class="py-1">
                                <a href="{{ route('profile.edit') }}"
                                    class="flex items-center gap-2 px-4 py-2 text-sm text-white/60 hover:text-white hover:bg-white/5 transition-colors">
                                ...
                                @elseif(auth()->user()->isPendingSeller())
                                    <a href="{{ route('seller.status') }}"
                                        class="flex items-center gap-2 px-4 py-2 text-sm text-yellow-400/80 hover:text-yellow-300 hover:bg-white/5 transition-colors">
                                        <i class="ti ti-clock text-sm"></i> Status Verifikasi
                                    </a>
                                @else
                                    <a href="{{ route('seller.register') }}"
                                        class="flex items-center gap-2 px-4 py-2 text-sm text-[#ff7a7a] hover:text-white hover:bg-white/5 transition-colors">
                                ...
                                @if (auth()->user()->isAdmin())
                                    <div class="border-t border-white/5 mt-1 pt-1">
                                        <a href="{{ route('admin.users.index') }}"
                                            class="flex items-center gap-2 px-4 py-2 text-sm text-purple-400 hover:text-purple-300 hover:bg-white/5 transition-colors">
                            ...
                            <div class="border-t border-white/5 py-1">
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit"
                                        class="w-full flex items-center gap-2 px-4 py-2 text-sm text-white/40 hover:text-red-400 hover:bg-white/5 transition-colors">
    ...
    @if (session('success'))
        <div
            class="bg-green-900/40 border border-green-500/20 text-green-300 text-sm px-6 py-3 flex items-center gap-2">
    ...
    <footer class="bg-[#0a0305] border-t border-white/5 mt-16">
        <div class="max-w-7xl mx-auto px-6 py-12 grid grid-cols-4 gap-8">
            ...
            <div>
                ...
                <ul class="space-y-2">
                    <li><a href="{{ route('home') }}"
                            class="text-sm text-white/35 hover:text-white transition-colors">Semua Iklan</a></li>
                    @foreach ($categories->take(4) as $cat)
                        <li><a href="{{ route('listings.category', $cat->slug) }}"
                                class="text-sm text-white/35 hover:text-white transition-colors">{{ $cat->name }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
            <div>
                ...
                <ul class="space-y-2">
                    <li><a href="{{ route('seller.register') }}"
                            class="text-sm text-white/35 hover:text-white transition-colors">Daftar Jadi Penjual</a>
                    </li>
                    <li><a href="{{ route('listings.create') }}"
                            class="text-sm text-white/35 hover:text-white transition-colors">Pasang Iklan</a></li>
                    @auth
                        @if (auth()->user()->isSeller())
                            <li><a href="{{ route('dashboard') }}"
                                    class="text-sm text-white/35 hover:text-white transition-colors">Dashboard</a></li>
                        @endif
                    @endauth
                </ul>
            </div>
            <div>
                ...
                <ul class="space-y-2">
                    <li><a href="#" class="text-sm text-white/35 hover:text-white transition-colors">Cara
                            Beli</a></li>
                    <li><a href="#" class="text-sm text-white/35 hover:text-white transition-colors">Cara
                            Jual</a></li>
                    <li><a href="#"
                            class="text-sm text-white/35 hover:text-white transition-colors">Kebijakan</a></li>
                </ul>
            </div>
        </div>
